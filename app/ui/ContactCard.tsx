...
export default function ContactCard({
  ...
  return (
    <div className="relative flex w-[90%] md:w-[70%] m-auto h-auto justify-between border-2 border-white min-[320px]:flex-col laptop:flex-row laptop:w-full lg:w-[90%] xl:w-[70%]">
      <p className="w-full font-semibold text-2xl p-5 xs:text-lg md:flex-1 md:text-3xl border-b-2 md:border-b-2 md:border-r-0 md:p-7 laptop:border-b-0 laptop:border-r-2 laptop:text-left xl:p-14 uppercase text-center">
        {title}
      </p>
      ...
      <Link
        data-link="link"
        href={link}
        className="relative buttonAction w-full md:flex-1 text-2xl md:text-3xl flex items-center overflow-hidden group"
      >
        {/* background anim hover */}
        <span className="absolute inset-0 bg-corail scale-x-0 origin-left transition-transform duration-500 ease-in-out group-hover:scale-x-100 z-0" />
